...
1. un modello rapido produce una mappa con un contratto dedicato che contiene
   soltanto contesto, aree e processi, senza generare prematuramente dettagli;
2. ogni processo viene approfondito in due frammenti piccoli: Haiku ricostruisce
   attori, sistemi, input e sequenza; Sonnet ricostruisce decisioni, eccezioni,
   escalation, metriche e conoscenza mancante;
...
4. i processi completati vengono salvati in un checkpoint SQLite: dopo un
   errore, il nuovo tentativo paga ed elabora solo quelli mancanti finché il
   database dell’ambiente rimane disponibile;
...
essere aggiunti in seguito dalla Memoria Operativa con la stessa pipeline mirata,
evitando di ricostruire l’intera azienda. La schermata mostra fasi reali,
